import { Mail, ScrollText, UserCircle2 } from "lucide-react";
...
const StudentCard = ({ student }) => {
  ...
  return (
    <article className="flex h-full flex-col rounded-2xl border border-[var(--border-subtle)] bg-[var(--bg-card)] p-5 shadow-sm transition-shadow hover:shadow-md">
      <div className="flex items-start gap-4">
        ...
        <div className="min-w-0"><h2 className="text-lg font-bold leading-snug text-[var(--text-main)]">{name || "Estudiante"}</h2><span className="mt-2 inline-block rounded-full bg-[var(--color-ingenieria)]/10 px-2.5 py-1 text-xs font-semibold text-ingenieria">{program}</span></div>
      </div>
      {student.thesis_topic && <div className="mt-6 rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-surface)]/60 p-4"><p className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wide text-[var(--text-main)]/55"><ScrollText className="h-4 w-4 text-ingenieria" />Tema de tesis</p><p className="mt-2 line-clamp-4 text-sm leading-6 text-[var(--text-main)]/80">{student.thesis_topic}</p></div>}
