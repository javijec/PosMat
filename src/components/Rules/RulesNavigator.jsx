  ...
  return (
    <aside className="min-w-0 rounded-xl border border-gray-200 bg-white p-3 shadow-sm lg:sticky lg:top-6" aria-label="Secciones del reglamento">
      ...
      <div className="min-w-0 flex gap-2 lg:hidden">
        ...
        <button type="button" onClick={onPrev} disabled={selectedIndex === 0} className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg border border-gray-300 text-gray-700 transition hover:bg-gray-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ingenieria/30 disabled:cursor-not-allowed disabled:opacity-40" aria-label="Sección anterior"><ChevronLeft className="h-4 w-4" /></button>
        <button type="button" onClick={onNext} disabled={selectedIndex === data.length - 1} className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg border border-gray-300 text-gray-700 transition hover:bg-gray-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ingenieria/30 disabled:cursor-not-allowed disabled:opacity-40" aria-label="Sección siguiente"><ChevronRight className="h-4 w-4" /></button>
      </div>
        ...
        {data.map((section, index) => {
          ...
          return (
            <button
              ...
              className={`flex w-full gap-3 rounded-lg px-3 py-2.5 text-left text-sm transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ingenieria/30 ${
                isSelected ? "bg-ingenieria text-white shadow-sm" : "text-gray-700 hover:bg-gray-100"
              }`}
            >
              <span className={`mt-0.5 text-xs font-semibold ${isSelected ? "text-white/85" : "text-gray-500"}`}>
