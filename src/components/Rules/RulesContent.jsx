...
const RulesContent = ({ selectedSection, current, total, onPrev, onNext, canGoPrev, canGoNext }) => (
  <article className="rounded-xl border border-gray-200 bg-white shadow-sm">
    ...
    <footer className="flex flex-col gap-3 border-t border-gray-100 px-6 py-5 sm:flex-row sm:justify-between md:px-8">
      <button onClick={onPrev} disabled={!canGoPrev} className="inline-flex items-center justify-center gap-1 rounded-lg border border-gray-300 px-4 py-2 font-medium text-gray-700 transition hover:bg-gray-50 disabled:cursor-not-allowed disabled:opacity-40">
        <ChevronLeft className="h-4 w-4" /> Anterior
      </button>
      <button onClick={onNext} disabled={!canGoNext} className="inline-flex items-center justify-center gap-1 rounded-lg bg-ingenieria px-4 py-2 font-medium text-white transition hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-40">
