    ...
    <div className="min-h-screen bg-[var(--bg-main)]">
      <section className="border-b border-[var(--border-subtle)] bg-[var(--bg-surface)]">
        <div className="max-w-6xl mx-auto px-4 py-10 md:py-12">
          ...
          <h1 className="max-w-3xl text-3xl md:text-4xl font-bold text-[var(--text-main)] leading-tight">
            Todas las novedades del Posgrado en Ciencia de Materiales
          </h1>
          <p className="mt-3 max-w-2xl text-sm md:text-base text-[var(--text-main)]/75 leading-6">
            ...
            {[1, 2, 3].map((item) => (
              ...
              >
                ...
                <div className="space-y-3 p-5">
                  ...
                  <div className="h-7 w-2/3 animate-pulse rounded bg-black/5" />
          ...
          <div className="space-y-5">
            {news.map((item, index) => (
              <article
                key={item.id}
                id={`news-${item.id}`}
                className="overflow-hidden rounded-2xl border border-[var(--border-subtle)] bg-[var(--bg-card)] shadow-sm"
              >
                <div className="grid lg:grid-cols-[1.15fr_1fr]">
                  ...
                  <div className="p-5 md:p-6">
                    ...
                    <h2 className="text-2xl font-bold leading-tight text-[var(--text-main)]">
                      {item.title}
                    </h2>

                    <p className="mt-3 text-sm leading-6 text-[var(--text-main)]/75 md:text-base">
                      {item.summary}
                    </p>

                    {item.content ? (
                      <div className="mt-4 whitespace-pre-line text-sm text-[var(--text-main)]/80 leading-6">
                        {item.content}
                      </div>
                    ) : null}
                  </div>
